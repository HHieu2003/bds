    ...
    @if($yeuThichs->count() > 0)
        <div class="row g-4">
            @foreach($yeuThichs as $item)
                @if($item->batDongSan) <div class="col-md-6 col-lg-4">
                    <div class="card h-100 border-0 shadow-sm rounded-4 overflow-hidden position-relative">
                        <button onclick="toggleFavorite({{ $item->batDongSan->id }}); this.closest('.col-md-6').remove();" class="btn btn-sm btn-danger position-absolute top-0 end-0 m-2 rounded-circle" style="z-index: 10;" title="Bỏ lưu">
                            <i class="fas fa-times"></i>
                        </button>

                        <div class="position-relative">
                            <img src="{{ asset($item->batDongSan->hinh_anh) }}" class="card-img-top" style="height: 220px; object-fit: cover;">
                        ...
                        <div class="card-body">
                            <h5 class="card-title fw-bold text-truncate">
                                <a href="{{ route('home.show', $item->batDongSan->slug) }}" class="text-decoration-none text-dark">
                        ...
                        <div class="card-footer bg-white border-0 pb-3">
                            <a href="{{ route('home.show', $item->batDongSan->slug) }}" class="btn btn-primary w-100 rounded-pill">Xem Chi Tiết</a>
                        </div>
                    </div>
                </div>
                @endif
