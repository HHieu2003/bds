    ...
    <section class="position-relative d-flex align-items-center justify-content-center"
        ...
        <div class="container text-center text-white" data-aos="zoom-in">
            ...
            <form action="{{ route('frontend.du-an.index') }}" method="GET"
                class="bg-white p-2 rounded-pill shadow-lg mx-auto" style="max-width: 600px;">
                <div class="input-group">
                    <select name="khu_vuc" class="form-select border-0 bg-transparent fw-bold"
                        style="color: #475569; max-width: 200px; box-shadow: none;">
                    ...
                    <input type="text" name="tu_khoa" class="form-control border-0 bg-transparent shadow-none"
                        placeholder="Tên dự án..." value="{{ request('tu_khoa') }}">
                    <button type="submit" class="btn rounded-pill fw-bold px-4"
                        style="background-color: #FF8C42; color: white;">
            ...
            <div class="d-flex justify-content-between align-items-center mb-5 border-bottom pb-3">
                <h3 class="fw-bold serif-font text-dark mb-0">
                    @if (request('khu_vuc') && is_numeric(request('khu_vuc')))
                        Dự án tại: <span
                            style="color: #FF8C42;">{{ $khuVucs->where('id', request('khu_vuc'))->first()->ten_khu_vuc ?? 'Khu vực này' }}</span>
                    @else
                        Tất cả Dự án
                    @endif
                </h3>
            ...
            <div class="row g-4">
                        ...
                        <div class="card border-0 rounded-4 overflow-hidden shadow-sm h-100 position-relative hover-up cursor-pointer"
                            ...
                            <div class="overflow-hidden position-relative w-100" style="height: 300px;">
                                <img src="{{ $da->hinh_anh_dai_dien ? asset('storage/' . $da->hinh_anh_dai_dien) : 'https://vinhomesland.vn/wp-content/uploads/2023/10/be-boi-the-canopy-residences-vinhomes-smart-city.jpg' }}"
                                    ...
                                    style="object-fit: cover; transition: transform 0.6s ease;" alt="{{ $da->ten_du_an }}">

                                {{-- Overlay Gradient Đen bọc chữ --}}
                                <div class="position-absolute bottom-0 start-0 w-100 p-4"
                                    style="background: linear-gradient(to top, rgba(15,23,42,0.9) 0%, rgba(15,23,42,0.5) 60%, transparent 100%);">
                                    ...
                                    <small class="text-light opacity-75 d-block text-truncate">
                                        <i class="fa-solid fa-location-dot me-1" style="color: #FF8C42;"></i>
                                        {{ $da->dia_chi }}
                                    </small>
                                </div>
                            </div>
                ...
                @empty
                    <div class="col-12 text-center py-5">
                        ...
                        <a href="{{ route('frontend.du-an.index') }}" class="btn rounded-pill px-4 fw-bold mt-2"
                            style="background-color: #FF8C42; color: white;">
                            Xem tất cả dự án
                        </a>
                    </div>
