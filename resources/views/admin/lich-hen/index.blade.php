...
<style>
/* ── Status badges ── */
.lh-badge { display:inline-flex;align-items:center;gap:.3rem;padding:.25rem .65rem;border-radius:20px;font-size:.72rem;font-weight:700;white-space:nowrap; }
.lh-moi_dat      { background:#fff3cd;color:#856404; }
.lh-cho_xac_nhan { background:#cfe2ff;color:#084298; }
.lh-da_xac_nhan  { background:#d1e7dd;color:#0a3622; }
.lh-hoan_thanh   { background:#d1fae5;color:#065f46; }
.lh-tu_choi      { background:#fde8e8;color:#991b1b; }
.lh-huy          { background:#f3f4f6;color:#6b7280; }

/* ── Stat cards ── */
.stat-cards { display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:.75rem;margin-bottom:1.25rem; }
.stat-card  { background:#fff;border:1px solid var(--border);border-radius:10px;padding:.85rem 1rem;text-align:center;cursor:pointer;transition:all .2s; }
.stat-card:hover,.stat-card.active { border-color:var(--primary);box-shadow:0 0 0 3px rgba(var(--primary-rgb),.12); }
.stat-card .sc-num  { font-size:1.6rem;font-weight:800;line-height:1; }
.stat-card .sc-lbl  { font-size:.7rem;color:var(--text-sub);margin-top:.2rem; }

/* ── Table ── */
.lh-table th { font-size:.72rem;font-weight:700;color:var(--text-sub);background:#f8f9fa;white-space:nowrap; }
.lh-table td { vertical-align:middle;font-size:.82rem; }

/* ── Timeline badge ── */
.src-badge { font-size:.65rem;border-radius:4px;padding:.1rem .4rem;background:#e9ecef;color:#555; }
.src-website { background:#dbeafe;color:#1d4ed8; }
.src-sale    { background:#ede9fe;color:#5b21b6; }
.src-phone   { background:#dcfce7;color:#166534; }
.src-chat    { background:#fef9c3;color:#854d0e; }

/* ── Action row ── */
.lh-actions .btn { font-size:.75rem;padding:.3rem .7rem; }

/* ── Filter bar ── */
.filter-bar { background:#fff;border:1px solid var(--border);border-radius:10px;padding:.85rem 1rem;margin-bottom:1rem;display:flex;flex-wrap:wrap;gap:.6rem;align-items:flex-end; }
</style>
@endpush
...
<div class="container-fluid py-3">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        <div>
            ...
            <p class="text-muted mb-0" style="font-size:.78rem">
                @if($nhanVien->isSale()) Lịch hẹn bạn phụ trách
                @elseif($nhanVien->isNguonHang()) Lịch hẹn cần bạn xác nhận
                @else Tất cả lịch hẹn trong hệ thống
                @endif
            </p>
        </div>
    ...
    <form method="GET" class="filter-bar">
        ...
        <div>
            ...
            <input type="date" name="ngay" value="{{ request('ngay') }}" class="form-control form-control-sm" style="width:150px">
        </div>
        @if($nhanVien->isAdmin())
        <div>
            ...
            <select name="nguon_hang" class="form-select form-select-sm" style="width:160px">
                ...
                <option value="{{ $nv->id }}" {{ request('nguon_hang')==$nv->id ? 'selected' : '' }}>{{ $nv->ho_ten }}</option>
                @endforeach
            </select>
        </div>
        @endif
                ...
                <table class="table table-hover lh-table mb-0">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <div class="text-muted" style="font-size:.72rem">{{ $lh->sdt_khach_hang }}</div>
                        ...
                        <td>
                            ...
                            <div class="text-muted" style="font-size:.72rem">{{ $lh->batDongSan->ma_can }}</div>
                            @endif
                        </td>
                        ...
                        <td>
                            ...
                            <div class="text-muted" style="font-size:.72rem">{{ $lh->thoi_gian_hen->format('d/m/Y') }}</div>
                            @if($lh->thoi_gian_hen->isToday())
                                <span class="badge bg-warning text-dark" style="font-size:.6rem">Hôm nay</span>
                        ...
                        <td>
                            ...
                            $badgeMap = [
                                ...
                                'cho_xac_nhan'  => ['lh-cho_xac_nhan', 'fas fa-clock',        'Chờ xác nhận'],
                                'da_xac_nhan'   => ['lh-da_xac_nhan',  'fas fa-check-circle', 'Đã xác nhận'],
                            ...
                            [$cls,$ico,$lbl] = $badgeMap[$lh->trang_thai] ?? ['lh-huy','fas fa-question','?'];
                            @endphp
                            <span class="lh-badge {{ $cls }}"><i class="{{ $ico }}"></i>{{ $lbl }}</span>
                            @if($lh->trang_thai === 'tu_choi' && $lh->ly_do_tu_choi)
                                <div class="text-danger mt-1" style="font-size:.68rem" title="{{ $lh->ly_do_tu_choi }}">
                                    <i class="fas fa-info-circle"></i> {{ Str::limit($lh->ly_do_tu_choi, 40) }}
                                </div>
                            @endif
                        </td>
                        ...
                        <td class="lh-actions">
                            <div class="d-flex flex-wrap gap-1">
                                ...
                                {{-- Xem chi tiết --}}
                                <a href="{{ route('nhanvien.admin.lich-hen.show', $lh) }}" class="btn btn-outline-secondary btn-sm" title="Xem chi tiết">
                                    <i class="fas fa-eye"></i>
                                </a>

                                {{-- SALE: Tiếp nhận lịch từ KH (moi_dat) --}}
                                @if($lh->trang_thai === 'moi_dat' && $nhanVien->hasRole(['admin','sale']))
                                <button class="btn btn-warning btn-sm" title="Tiếp nhận & phân công Nguồn hàng"
                                ...
                                <form action="{{ route('nhanvien.admin.lich-hen.xac-nhan', $lh) }}" method="POST" class="d-inline">
                                    @csrf @method('PATCH')
                                ...
                                <form action="{{ route('nhanvien.admin.lich-hen.hoan-thanh', $lh) }}" method="POST" class="d-inline">
                                    @csrf @method('PATCH')
    ...
    <form action="" method="POST" id="formTiepNhan">
      ...
      <div class="modal-content">
        <div class="modal-header bg-warning-subtle">
          <h5 class="modal-title"><i class="fas fa-user-check me-2"></i>Tiếp nhận & Phân công Nguồn hàng</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
          <p class="text-muted mb-3">Lịch hẹn của: <strong id="tnKhach"></strong></p>
          <div class="mb-3">
            <label class="form-label fw-600">Nhân viên Nguồn hàng <span class="text-danger">*</span></label>
          ...
          <div class="mb-3">
            <label class="form-label fw-600">Thời gian hẹn xác nhận <span class="text-danger">*</span></label>
            <input type="datetime-local" name="thoi_gian_hen" id="tnThoiGian" class="form-control" required>
          </div>
          <div class="mb-3">
            <label class="form-label fw-600">Địa điểm</label>
            <input type="text" name="dia_diem_hen" class="form-control" placeholder="Địa chỉ xem nhà...">
          </div>
          <div class="mb-0">
