        ...
        <div class="row g-2 align-items-end">
            <div class="col-12 col-md-3 col-xl-2">
                ...
                <select name="loai_loc" class="form-select form-select-sm" id="selLoaiLoc" onchange="toggleCustom()">
                    <option value="ngay" {{ $loai_loc == 'ngay' ? 'selected' : '' }}>Theo ngày</option>
                    <option value="tuan" {{ $loai_loc == 'tuan' ? 'selected' : '' }}>Tuần này</option>
                    <option value="thang" {{ $loai_loc == 'thang' ? 'selected' : '' }}>Theo tháng</option>
                    <option value="nam" {{ $loai_loc == 'nam' ? 'selected' : '' }}>Theo năm</option>
                    <option value="tuy_chon" {{ $loai_loc == 'tuy_chon' ? 'selected' : '' }}>Tuỳ chỉnh...</option>
                </select>
               
            </div>

            <div class="col-12 col-md-3 col-xl-2 filter-extra" id="wrapNgay"
                style="display:{{ $loai_loc == 'ngay' ? 'block' : 'none' }};">
                <label class="form-label small fw-bold mb-1">Chọn ngày</label>
                <input type="date" name="ngay" class="form-control form-control-sm" value="{{ $ngay_input }}">
            </div>

            <div class="col-12 col-md-3 col-xl-2 filter-extra" id="wrapThang"
                style="display:{{ $loai_loc == 'thang' ? 'block' : 'none' }};">
                <label class="form-label small fw-bold mb-1">Chọn tháng</label>
                <input type="month" name="thang" class="form-control form-control-sm" value="{{ $thang_input }}">
            </div>

            <div class="col-12 col-md-2 col-xl-2 filter-extra" id="wrapNam"
                style="display:{{ $loai_loc == 'nam' ? 'block' : 'none' }};">
                <label class="form-label small fw-bold mb-1">Chọn năm</label>
                <input type="number" name="nam" min="2020" max="2099" class="form-control form-control-sm"
                    value="{{ $nam_input }}">
            </div>

            <div class="col-12 col-xl-4 d-flex gap-2 align-items-end filter-extra" id="customRange"
                style="display:{{ $loai_loc == 'tuy_chon' ? 'flex' : 'none' }};">
                <div>
                ...
                <div>
                    ...
                    <input type="date" name="den_ngay" class="form-control form-control-sm"
                        value="{{ $den_ngay_input }}">
                </div>
            </div>

            <div class="col-12 col-md-auto">
                <button type="submit" class="btn btn-primary btn-sm"><i class="fas fa-search me-1"></i>Áp dụng</button>
                <a href="{{ route('nhanvien.thong-ke') }}" class="btn btn-outline-secondary btn-sm ms-1"><i
                        class="fas fa-undo"></i></a>
                <a href="{{ route('nhanvien.thong-ke', $exportQuery) }}" class="btn btn-success btn-sm ms-1"><i
                        class="fas fa-file-export me-1"></i>Xuất báo cáo</a>
            </div>
        </div>
    </form>
        ...
        @foreach ($kpis as $k)
            <div class="col-6 col-xl-3">
                <div class="tk-card kpi-card h-100">
                    <div class="kpi-icon" style="background:{{ $k['bg'] }};color:{{ $k['ic'] }}"><i
                            class="fas {{ $k['icon'] }}"></i></div>
                    <div>
        ...
        @php
            $kpi2 = [
                ...
                [
                    ...
                    'prev' => $bdsPrev,
                    'delta' =>
                        $bdsPrev > 0 ? round((($bdsNow - $bdsPrev) / $bdsPrev) * 100, 1) : ($bdsNow > 0 ? 100 : 0),
                    'label' => 'BĐS mới (' . $label . ')',
                    'icon' => 'fa-home',
                    'bg' => '#fdf4ff',
                    'ic' => '#9333ea',
                ...
                <div class="tk-card kpi-card h-100">
                    ...
                    <div>
                        ...
                        <div class="kpi-delta {{ $up ? 'up' : 'down' }}">
                            <i class="fas fa-arrow-{{ $up ? 'up' : 'down' }}"></i> {{ abs($k['delta']) }}% so với kỳ trước
                            ({{ number_format($k['prev']) }})
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="tk-card h-100">
                ...
                <div class="rank-list" style="max-height:230px">
                    @forelse($trangNoiBat as $i => $t)
                        <div class="rank-row">
                            <div class="rank-num {{ $i == 0 ? 'g1' : ($i == 1 ? 'g2' : ($i == 2 ? 'g3' : '')) }}">
                                {{ $i + 1 }}</div>
                            <div class="rank-bar-wrap">
                                <span class="rank-name">{{ $t['trang'] }}</span>
                                <div class="rank-bar">
                                    <div class="rank-fill"
                                        style="width:{{ round(($t['luot'] / $maxTrang) * 100) }}%;background:linear-gradient(90deg,#f59e0b,#ef4444)">
                                    </div>
                                </div>
                            </div>
                    ...
                    @forelse($khuVucNoiBat as $i => $kv)
                        <div class="rank-row">
                            ...
                            <div class="rank-bar-wrap">
                                ...
                                <div class="rank-bar">
                                    <div class="rank-fill"
                                        style="width:{{ round(($kv->luot_xem / $maxKV) * 100) }}%;background:linear-gradient(90deg,#6366f1,#8b5cf6)">
                                    </div>
                                </div>
                            </div>
                    ...
                    @forelse($duAnNoiBat as $i => $da)
                        <div class="rank-row">
                            ...
                            <div class="rank-bar-wrap">
                                ...
                                <div class="rank-bar">
                                    <div class="rank-fill"
                                        style="width:{{ round(($da->luot_xem / $maxDA) * 100) }}%;background:linear-gradient(90deg,#06b6d4,#3b82f6)">
                                    </div>
                                </div>
                            </div>
                    ...
                    @forelse($bdsNoiBat as $i => $b)
                        <div class="rank-row">
                            <div class="rank-num {{ $i == 0 ? 'g1' : ($i == 1 ? 'g2' : ($i == 2 ? 'g3' : '')) }}">
                                {{ $i + 1 }}</div>
                            <div class="rank-bar-wrap">
                                <a href="{{ route('nhanvien.admin.bat-dong-san.edit', $b->id) }}"
                                    class="rank-name text-dark text-decoration-none">{{ Str::limit($b->tieu_de, 45) }}</a>
                                <div class="rank-bar">
                                    <div class="rank-fill"
                                        style="width:{{ round(($b->luot_xem / $maxBDS) * 100) }}%;background:linear-gradient(90deg,#10b981,#06b6d4)">
                                    </div>
                                </div>
                            </div>
        ...
        <div class="col-12 col-xl-5">
            <div class="tk-card h-100">
                ...
                <div class="row g-2 mb-3 text-center">
                    <div class="col-4">
                        <div class="fw-bold fs-5 text-primary">{{ $lichHenNow }}</div>
                        <div class="kpi-label">Kỳ này</div>
                    </div>
                    <div class="col-4">
                        <div class="fw-bold fs-5 text-success">{{ $lichHenHoanThanh }}</div>
                        <div class="kpi-label">Hoàn thành</div>
                    </div>
                    <div class="col-4">
                        <div class="fw-bold fs-5 text-warning">{{ $lichHenChoXuLy }}</div>
        ...
        <div class="col-12 col-xl-3">
            <div class="tk-card h-100">
                ...
                <div class="text-center mb-3">
                    ...
                    <div class="kpi-delta {{ $lu ? 'up' : 'down' }}"><i
                            class="fas fa-arrow-{{ $lu ? 'up' : 'down' }}"></i>
                        {{ abs($leadsDelta) }}% vs kỳ trước</div>
                </div>
                <hr class="my-2">
                <div class="d-flex justify-content-around text-center">
                    <div>
                        <div class="fw-bold fs-5 text-danger">{{ $leadsChuaXuLy }}</div>
                        <div class="kpi-label">Chưa xử lý</div>
                    </div>
                    <div>
                        <div class="fw-bold fs-5 text-secondary">{{ $leadsPrev }}</div>
                        <div class="kpi-label">Kỳ trước</div>
                    </div>
                </div>
                <a href="{{ route('nhanvien.admin.lien-he.index') }}" class="btn btn-outline-danger btn-sm w-100 mt-3"><i
                        class="fas fa-arrow-right me-1"></i>Xem tất cả Leads</a>
            </div>
        </div>
                    ...
                    @forelse($tuKhoaTop as $i => $tk)
                        <div class="rank-row">
                            ...
                            <div class="rank-bar-wrap">
                                ...
                                <div class="rank-bar">
                                    <div class="rank-fill"
                                        style="width:{{ round(($tk->so_lan / $maxTK) * 100) }}%;background:linear-gradient(90deg,#f59e0b,#f97316)">
                                    </div>
                                </div>
                            </div>
    ...
    <div class="row g-3 mb-4">
        @php
            $sysCards = [
                [
                    'num' => $bdsNow,
                    'label' => 'BĐS mới kỳ này',
                    'icon' => 'fa-home',
                    'bg' => '#dbeafe',
                    'ic' => '#2563eb',
                    'extra' => 'Tổng kho: ' . $bdsTong . ' | Còn hàng: ' . $bdsConHang,
                ],
                [
                    'num' => $chuNhaNow,
                    'label' => 'Chủ nhà mới',
        ...
        @foreach ($sysCards as $c)
            <div class="col-6 col-xl-3">
                <div class="tk-card kpi-card h-100">
                    <div class="kpi-icon" style="background:{{ $c['bg'] }};color:{{ $c['ic'] }}"><i
                            class="fas {{ $c['icon'] }}"></i></div>
                    <div>
...
@push('scripts')
    ...
    <script>
        function toggleCustom() {
            ...
            const wrapNgay = document.getElementById('wrapNgay');
            const wrapThang = document.getElementById('wrapThang');
            const wrapNam = document.getElementById('wrapNam');
            const customRange = document.getElementById('customRange');

            if (wrapNgay) wrapNgay.style.display = v === 'ngay' ? 'block' : 'none';
            if (wrapThang) wrapThang.style.display = v === 'thang' ? 'block' : 'none';
            if (wrapNam) wrapNam.style.display = v === 'nam' ? 'block' : 'none';
            if (customRange) customRange.style.display = v === 'tuy_chon' ? 'flex' : 'none';
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            new Chart(document.getElementById('chartLichHen'), {
                ...
                data: {
                    ...
                    datasets: [{
                            label: 'Tổng lịch',
                            data: {!! json_encode(array_column($chartLichHen6Thang, 'so_luong')) !!},
                            backgroundColor: '#bfdbfe',
                            borderRadius: 5
                        },
                        {
                            label: 'Hoàn thành',
                            data: {!! json_encode(array_column($chartLichHen6Thang, 'hoan_thanh')) !!},
